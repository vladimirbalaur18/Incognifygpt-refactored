(function () {
    ...
    window.incognifyInterceptorLoaded = true;
     ...
     let aiServiceRegistry = null;
    
     // Listen for registry initialization from content script
     window.addEventListener('message', (event) => {
         if (event.data?.type === 'INCOGNIFY_INIT_REGISTRY' && event.data.registry) {
             aiServiceRegistry = event.data.registry;
         }
     });
    ...
    window.fetch = async function (...args) {
        ...
        if (!aiServiceRegistry || typeof resource !== 'string' || !config?.body) {
            return originalFetch.apply(this, args);
        }

        try{
            const aiServiceAdapter = aiServiceRegistry.getAdapterForUrl(resource, config.method);
            ...
            const extractionResult = aiServiceAdapter.extractUserMessage(body);
            ...
            window.postMessage({
                type: 'INCOGNIFY_INTERCEPT_REQUEST',
                id: requestId,
